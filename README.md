...
> Charts are regenerated by `python scripts/generate_charts.py` (or `make charts`).
> Run the full pipeline first (`make pipeline`) to replace sample data with live metrics.

### Sentiment Confusion Matrices

| LexiconABSA | TransformerABSA | LLMABSA |
|:-----------:|:---------------:|:-------:|
| ![LexiconABSA confusion matrix](docs/images/confusion_matrix_lexicon.png) | ![TransformerABSA confusion matrix](docs/images/confusion_matrix_transformer.png) | ![LLMABSA confusion matrix](docs/images/confusion_matrix_llm.png) |

**Observations:**
- **LexiconABSA** frequently predicts *neutral* when uncertain — visible as a dark right column.
- **TransformerABSA** shows a balanced matrix with slight neutral bias.
- **LLMABSA** is highly confident; it rarely predicts *neutral* (~2% of outputs), so neutral ground-truth items scatter into positive/negative predictions.

### Precision / Recall / F1 and Latency

![Model comparison bar chart](docs/images/model_comparison.png)

| Model | Precision | Recall | F1 | Latency (avg) |
|-------|:---------:|:------:|:--:|:-------------:|
| LexiconABSA | 0.97 | 0.61 | 0.72 | ~0.02 s |
| TransformerABSA | 0.97 | 0.80 | 0.82 | ~0.50 s |
| LLMABSA | 0.98 | 0.86 | 0.89 | ~10 s |

### Aspect Detection Analysis

**Insights**:
- LexiconABSA has **highest precision** but misses many aspects (recall ≈ 61 %)
...
```bash
make pipeline   # runs all three analysers → data/evaluation_report.json
make charts     # regenerates docs/images/ PNGs from the report
make all        # pipeline + charts in one command
```
...
python scripts/generate_charts.py  # charts only
```
